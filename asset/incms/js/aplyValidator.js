...
var AplyValidator = function(){
	...
	_aply.init = function(){
		...
		_aply.numberAndZero = ".numberzero-area";
		_aply.accno = ".number-accno";
		_aply.bizrno = ".number-bizno";
		_aply.corgno = ".number-corgno";
		_aply.date = ".number-date";
		_aply.email = ".email-id";
		_aply.domain = ".email-domain";
	...
	_aply.actionEvent = function(){
		...
		$(document).on("change",_aply.accno,function(e){
			...
			var chkAcc = /^[0-9]+$/;
			if($(this).val().length > 15){
				if(typeof $($(this).addClass("caution").closest("td").html() !== "undefined")){
					thistag = $(this).addClass("caution").closest("td");
				}else{
					thistag = $(this).closest("li");
				}
				alert("계좌는 15자리까지 입력됩니다.");
				$(this).val("");
				return false;
	...
	_aply.valid = function(form){
		var value = new MiyaValidator(document.forms[form]);
		var defaultText = "";
		$.each($("#"+form).find("input,select,textarea"),function(i,v){
			
			if($(this).attr("type") == "hidden" && typeof $(this).data('text') !== 'undefined'){
				defaultText = $(this).data('text');
			}
			
			console.log(defaultText);
			if($("label[for ='"+$(this).attr("id")+"'] > .req").text() != ''){
				if($(this).attr("type") == "password"){
					value.add($(this).attr("id"),{required:true,option:"password",label:defaultText});	
				}else if($(this).attr("id").indexOf("email") > -1){
					value.add($(this).attr("id"),{required:true,option:"email",label:defaultText});
				}else if($(this).attr("id").indexOf("telno") > -1 ){
					if($(this).data("req-phone") == 'Y'){
						value.add($(this).attr("id"),{required:true,option: "phone",maxbyte:11,label:defaultText});						
					}else{
						value.add($(this).attr("id"),{option: "homephone",maxbyte:14,label:defaultText});
					}
				}else if($(this).attr("id").indexOf("faxno") > -1){
					value.add($(this).attr("id"),{option: "homephone",maxbyte:14,label:defaultText});
				}else{
					value.add($(this).attr("id"),{required:true,label:defaultText});
				}		
			}else{
					if($(this).attr("id").indexOf("email") > -1){
						...
						value.add($(this).attr("id"),{required:true,mincheck:1,label:"사용여부"});
					}
			}
		});
