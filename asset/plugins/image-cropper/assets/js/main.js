$(function () {
  ...
  if (typeof $download[0].download === 'undefined') {
    $download.addClass('data-none');
	// $download.attr('id','iedownLoad');
      ...
      switch (data.method) {
        ...
        case 'getCroppedCanvas':
          if (result) {
            ...
            if (!$download.hasClass('data-none')) {
				
              $download.attr('href', result.toDataURL());
			  //console.log(result.toDataURL());
            }else{
				$download.attr('id','ieDownloadBtn');
			}
          }
  ...
  $('.data-none').on('click',function(){
    ...
    globals.canvas.context.imageDataObject = ctx.createImageData(canvasWidth, canvasHeight); // Create an appropriately sized but empty canvas image 
	 ...
	 if (window.BlobBuilder && globals.canvas.toBlob && window.navigator.saveBlob) {
        var extensionlessFilename = "main_image";
