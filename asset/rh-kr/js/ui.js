	...
	setTimeout(function () { // 개발 적용 시 setTimeout 삭제
		$(".btn_quick_menu").on("click", function () {
			...
			if ($(this).hasClass("close")) {
				$(".quick_menu").css("transform", "translateX(0)");
			} else {
				$(".quick_menu").css("transform", "translateX(calc(100% - 32px))");
			}
		})
	}, 300)
...
$(function () {
	...
	if ($('.srh_header').length) {
		$(window).on('load scroll', function () {
			if ($(document).scrollTop()) {
				$('.srh_header').addClass('scrolled')
			} else {
				$('.srh_header').removeClass('scrolled')
			}
		})
	}
	//통합검색 상세

	$('.srh_opt .btn_expand').on('click', function () {
		$(this).parents('.srh_opt').toggleClass('expand');
		$('.opt_wrap').slideToggle();
	})
})